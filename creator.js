...
function generateLabelTable(data) {
  ...
  let html = `
    <table style="width: 100%; border-collapse: collapse; border: 2px solid #000; font-family: Arial, sans-serif;">
      <tr>
        <td style="border: 1px solid #000; padding: 8px; width: 35%; font-weight: bold; background: #f9f9f9;">제품명</td>
        <td style="border: 1px solid #000; padding: 8px;">${productName || '-'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">식품유형</td>
        <td style="border: 1px solid #000; padding: 8px;">${foodType || '-'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">소비기한</td>
        <td style="border: 1px solid #000; padding: 8px;">${consumerDate || '-'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">내용량</td>
        <td style="border: 1px solid #000; padding: 8px;">${weight || '-'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">제조원</td>
        <td style="border: 1px solid #000; padding: 8px;">${manufacturer || '-'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">소재지</td>
        <td style="border: 1px solid #000; padding: 8px;">${manufacturerAddress || '-'}</td>
      </tr>
  `;
  ...
  if (ingredientText) {
    html += `
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">원재료명</td>
        <td style="border: 1px solid #000; padding: 8px;">${ingredientText}</td>
      </tr>
    `;
  }
  ...
  if (allergenText) {
    html += `
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #fff3cd; color: #856404;">알레르기</td>
        <td style="border: 1px solid #000; padding: 8px; background: #fff3cd;">${allergenText}</td>
      </tr>
    `;
  }
  ...
  html += `
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">보관방법</td>
        <td style="border: 1px solid #000; padding: 8px;">${storageMethod || '-'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 8px; font-weight: bold; background: #f9f9f9;">포장재질</td>
        <td style="border: 1px solid #000; padding: 8px;">${packagingMaterial || '-'}</td>
      </tr>
      <tr>
...
function downloadLabel(format) {
  ...
  const filename = formData.productName || 'label';
  
  if (format === 'pdf') {
    downloadPDF(filename, labelHTML);
  } else if (format === 'png') {
    downloadPNG(filename, labelHTML);
  }
...
function downloadPNG(filename, htmlContent) {
  const element = document.createElement('div');
  element.innerHTML = htmlContent;
  element.style.padding = '20px';
  element.style.backgroundColor = 'white';
...
function copyToClipboard() {
  const ingredientLabel = formData.ingredients.map(i => i.label).join(', ');
  const allergenLabel = formData.allergens.length > 0 
    ? `알레르기 유발물질: ${formData.allergens.join(', ')} 함유`
    : '';
  
  let text = formData.productName || '라벨 정보';
  if (ingredientLabel) text += '\n원재료명: ' + ingredientLabel;
  if (allergenLabel) text += '\n' + allergenLabel;
...
function loadFromLocalStorage() {
  const saved = localStorage.getItem('creatorFormData');
  if (saved) {
    try {
      ...
      // 업종 복원
      const businessTypeRadio = document.querySelector(`input[name="businessType"][value="${formData.businessType}"]`);
      if (businessTypeRadio) businessTypeRadio.checked = true;
